...
// Dev-only: serve the app at /puzzle/ too. In production this path is a real
// file (dist/puzzle/index.html, generated by scripts/build-share-page.js with
// the social-embed block stripped); in dev we just serve the transformed
// index.html so /puzzle/?date=... links work.
function shareRoute(): Plugin {
  return {
    name: 'serve-share-route',
    apply: 'serve',
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (!req.url || !/^\/puzzle\/?(?:[?#]|$)/.test(req.url)) return next();
        if (req.method !== 'GET' && req.method !== 'HEAD') return next();
...
export default defineConfig({
  base: '/',
  // 'mpa' disables the dev server's SPA fallback (serving index.html for any
  // unknown path), which GitHub Pages doesn't do either. Without this, dev
  // couldn't tell a real route like /puzzle/ from a nonexistent one.
  appType: 'mpa',
  plugins: [experimentalLevels(), watchWordsAndLevels(), shareRoute()],
});
